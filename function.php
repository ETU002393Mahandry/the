<?php
    ...
    function getNbPieds($surface, $occupation){
        $surface = hectaresEnMetresCarres($surface);
        $nbpied = $surface / $occupation;
        return $nbpied;
    }

    function getnecessaryPrevisionGlobal($bdd, $date) {
        $resultat = array();
        $resultat['restethe'] = getPoidsRestant($bdd, '2023-01-01', $date);
        $resultat['vente'] = getTotalPrixDeVente($bdd, '2023-01-01', $date);

        return $resultat;
    }

    function getnecessaryPrevisionParcelle($bdd, $date) {
        $resultat = array();
        $requeteParcelle = "SELECT idparcelle, surface, v.occupation AS occupation, v.rendement AS rendement, v.nomvariete AS nomvariete FROM the_Parcelle p INNER JOIN the_Variete v ON p.idvariete = v.idvariete";
        $result=mysqli_query($bdd,$requeteParcelle);
        $parcelles=array();
        while($data=mysqli_fetch_assoc($result))
        {
            $parcelles[]=$data;
        ...
        for ($i=0; $i < count($parcelles); $i++) { 
            $resultat[$i]['surface'] = $parcelles[$i]['surface'];
            $resultat[$i]['nbpieds'] = getNbPieds($parcelles[$i]['surface'], $parcelles[$i]['occupation']);
            $resultat[$i]['restant'] = poidsRestantSurParcelle($bdd, $parcelles[$i]['idparcelle'], '2023-01-01', $date);
            $resultat[$i]['nomvariete'] = $parcelles[$i]['nomvariete'];
            $resultat[$i]['idparcelle'] = $parcelles[$i]['idparcelle'];
        }
